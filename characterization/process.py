...
with open(name) as data:
    ...
    if 'draw' in argv:
        for l in range(maxLevel + 1):
            ...
            nx.draw_networkx_edges(G, pos, edgelist = E[l]['d'], width = 2, alpha = 0.5, edge_color='g')
            plt.axis('off')
            plt.title('{:s} {:d} internal'.format(name, l))
            plt.savefig('{:s}_{:d}_1.png'.format(name, l))
            plt.clf()
            if l < maxLevel:
                plt.figure(figsize=(12,12)) 
                nx.draw_networkx_nodes(G, pos, nodelist = F, node_color = 'b', node_size = 250, alpha = 0.5)
                ...
                plt.axis('off')
                plt.title('{:s} {:d} to {:d}'.format(name, l, l + 1))        
                plt.savefig('{:s}_{:d}_2.png'.format(name, l))        
                plt.clf()
    ...
    print('edge cover order', len(algs.covering.min_edge_cover(G)))
