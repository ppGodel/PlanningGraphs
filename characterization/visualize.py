...
assert (max(V.keys()) >= 4) # need some lvl 4 nodes
...
for l in range(lvlCount): # the first three levels
    ...
    print(len(F), 'facts')
    print(len(A), 'actions')
    nx.draw_networkx_nodes(G, pos, nodelist = F, node_color = 'b', node_size = 500, alpha = 0.9)
    nx.draw_networkx_nodes(G, pos, nodelist = A, node_color = 'g', node_size = 300, alpha = 0.9)
